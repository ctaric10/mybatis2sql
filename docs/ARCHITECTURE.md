...
渲染前会先进行不创建 SQL 变体的数量预判，并在每次可能扩大结果集时继续检查
`mybatis2sql.maxVariants`。预判超限时直接从语句根节点切换到线性渲染：静态节点
组成基线，每个动态条件只与基线合并，不同条件之间不做笛卡尔积。语句渲染完成后
使用 `LinkedHashMap` 按规范化 SQL 去重，因此输出顺序可复现。
