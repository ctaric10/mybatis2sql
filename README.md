...
- 默认限制单条语句的中间组合数；超限时自动改为逐条件生成，避免动态分支爆炸。
...
- 不计算 OGNL 条件之间的逻辑关系，因此输出可能包含业务上不可同时出现的组合。
- `foreach` 只用一个元素和两个元素代表非空集合，不枚举任意集合长度。
- 空 `foreach` 不生成输出，以免与外部静态文本组合成残缺 SQL。
- 工具用于静态分析，不保证生成的每个分支都能在特定数据库方言中直接执行。

默认单条语句最多枚举 10,000 个中间组合。预计结果超过上限时，程序会放弃该语句
的排列组合并从头按逐条件模式重新生成：每个条件单独生成一条 SQL，不与其他条件
组合；输出中会标注本次降级。静态 SQL 片段仍会保留。
